...
def get_stock_history(stock:str,period:str,interval:str)->pd.DataFrame:
    ...
    hist = hist.iloc[:,:-1] # remove last column
...
def get_exchange_rate(from_currency:str,to_currency:str,period:str,interval:str)->pd.DataFrame:
    ...
    fx_rates.columns = [col[0] for col in fx_rates.columns]  # Keep only the first level of the MultiIndex
    ...
    fx_rates = fx_rates[['Date', 'Ticker','From Currency', 'To Currency', 'Open', 'High', 'Low', 'Close', 'Volume']]
